...
class CVAE:
    ...
    def Conditional_Variational_AutoEncoder(self, X, X_noised, Y, keep_prob):
        ...
        likelihood = tf.reduce_mean(X_flatten*tf.log(X_out) + (1 - X_flatten)*tf.log(1 - X_out))
        KL_Div = tf.reduce_mean(0.5 * (1 - tf.log(tf.square(std) + 1e-8) + tf.square(mean) + tf.square(std)))
