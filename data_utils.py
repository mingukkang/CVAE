...
class data_pipeline:
    ...
    def make_noise(self,image):
        ...
        image = zero_masking_noise(salt_pepper_noise(image))

        return image

    def next_batch(self, images, labels, batch_size, make_noise = True):
        ...
        if make_noise is False:
            ...
            if self.type == "MNIST":
                ...
                batch_noised_xs = np.reshape(batch_xs, [-1, self.size, self.size, self.n_channels])
            else:
                for i in range(batch_size):
